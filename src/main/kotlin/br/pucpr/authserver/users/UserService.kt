...
class UserService(
    ...
    fun insert(user: User): User {
        ...
        log.info("User inserted: {}", savedUser.id)

        if (savedUser.avatar.isNullOrBlank() || savedUser.avatar == AvatarService.DEFAULT_AVATAR) {
            val newAvatar = avatarService.generateAutoAvatar(savedUser)
    ...
    fun toResponse(user: User): UserResponse =
        UserResponse(user, avatarService.urlFor(user.avatar) ?: AvatarService.DEFAULT_AVATAR)
